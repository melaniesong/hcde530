...
This week I used my **MP1 restaurant dataset** to understand the table beyond a quick glance. **My pandas work for this week is in `MP1 Restaurants/MP1 analysis A5.ipynb`** so a reader can open that file and follow the same steps.
...
Pandas made it **easy to summarize** a very large table quickly—for example **`value_counts`** for category frequencies and **`groupby` + `agg`** for counts and average ratings by time zone (see **`MP1 analysis A5.ipynb`**).
